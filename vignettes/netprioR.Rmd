...
```{r, echo = TRUE}
class.labels <- simulate_labels(values = c(+1, -1), sizes = members_per_class, 
                          nobs = c(nlabel/2, nlabel/2) %>% floor)
```
The list of simulated labels contains the complete vector of `r N` labels `Y`, indices
of labelled (`l`) and unlabelled (`u`) genes, as well as vector of observed labels, where
unlabelled genes are set to 0.
```{r, echo = TRUE}
lapply(class.labels, head)
```
...
G <- list(LOW_NOISE1 = simulate_network_scalefree(nmemb = members_per_class, pclus = 0.8),
...
```{r, echo = TRUE}
X <- simulate_phenotype(Y = class.labels$Y, meandiff = mdiff, sd = 1, pivot = 0.5)
```
...
as follows
```{r, echo = TRUE}
data.frame(Phenotype = X[,1], Class = rep(c("Positive", "Negative"), each = N/2)) %>%
  ggplot() +
...
First, we normalise the network data and compute the graph Laplacians of the
adjacency matrices.
```{r, echo = TRUE, cache = TRUE}
 G <- G %>% lapply(., function(x) x / norm(x))
 G <- lapply(G, laplacian, norm = "none")
```
Next, we call the `learn()` function for parameter estimation and imputation of
missing data (e.g. the missing labels) using the EM algorithm. We only pass the
vector of observed labels, `Yobs` and keep the true underlying labels `Y` for
performance evaluation.

In this example, we will use `a = b = 0.01` in order to yield a sparsifying prior on the estimated
network weights. We will fit only one model, setting `nrestarts` to 1, whereas in 
practise multiple restarts are used in order to avoid cases where the EM gets stuck
in local minima.
```{r, echo = TRUE, cache = TRUE}
netprioR <- learn(Yobs = class.labels$Yobs, X = X, G = G, 
                  l = class.labels$l, u = class.labels$u, 
                  a = 0.01, b = 0.01, max.iter = 50, use.cg = FALSE, 
                  nrestarts = 1, max.cores = 1, verbose = FALSE)
```

## Performance evaluation
First, we look at the estimated weights (i.e. importance) of each network data set
```{r, echo = TRUE}
data.frame(Network = names(netprioR$W), Weight = netprioR$W) %>%
  mutate(Weight = Weight / sum(Weight) * 100) %>%
  ggplot(aes(x = Network, y = Weight)) +
...
```{r, echo = TRUE}
auc.yimp <- AUC(pred = netprioR$Yimp, Y = class.labels$Y, ind = class.labels$u, plot = TRUE, binarise = FALSE)
```

In addition, we compute the ROC curve for the case where we prioritise soleley
based on the phenotype
```{r, echo = TRUE}
auc.x <- AUC(pred = X[,1], Y = class.labels$Y, ind = class.labels$u, plot = TRUE, binarise = FALSE)
```
...
and TNs, we gain about `r round(auc.yimp$auc - auc.x$auc, 2)` in AUC.
